...
- **Shuffle cards** — when on, the card order is randomised for this session. If the set's author has turned on [Enforce card order](sets.md#set-details), this option is replaced by a **Fixed card order** note and the set is always studied in the author's order — you can't shuffle it. (Study Review and Study Mistakes are never affected — they can always be shuffled.)
