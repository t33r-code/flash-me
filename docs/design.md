...
The pane's set-mode toolbar is **adaptive** (#234): when the pane is narrower than `kSetToolbarOverflowWidth` (520) the set-management actions (market/export/delete/edit) collapse into a single ⋮ overflow `PopupMenuButton`, keeping the builder actions (+, library, study) always visible so the AppBar action row can't overflow.
