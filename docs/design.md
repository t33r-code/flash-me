...
**Clone Card (#231 Flash Cards, #274 Workbook Cards)** — Clone opens the editor exactly as if creating a brand-new card, seeded from the source's fields (`primaryWord`/`translation`/`questions`/`tags`/language pair for Flash; `prompt`/`questions`/`tags`/language pair for Workbook) — it is a genuinely **unsaved draft**: nothing is written until the author hits Save, and cancelling creates nothing. Implementation-wise this is a single new `cloneFrom` parameter (`FlashCard?` / `WorkbookCard?`) threaded through `CardEditorBody`/`CardFormScreen` and `WorkbookEditorBody`/`WorkbookCardFormScreen` (plus `_FlashPaneEdit`/`_WorkbookPaneEdit` for the wide pane), seeded in `initState()` only when `card == null` — so the existing `_isEditing`/`_pendingCardId`/`_save()` branches are completely untouched and the clone naturally goes through `createCard`/`createCard` with a fresh id/`createdAt`/`createdBy`, never touching the source. Fresh `questionId`s per cloned question are generated via the shared top-level `withFreshQuestionId` (`models/card_question.dart`, promoted there in #274 so both editors re-key identically instead of each keeping a private copy). Workbook Cards have no media fields at all, so the media-orphaning question #231 had to resolve for Flash Cards (no reference counting on `deleteCard`, so a shared URL would let deleting either card break the other's — resolved by simply not copying media) doesn't apply to #274.
  - **Media is deliberately not carried over** — the clone's `primaryImageUrl`/`primaryAudioUrl` start null. `deleteCard` has no reference counting, so a shared Storage URL between a card and its clone would mean deleting either one breaks the other's media; duplicating the file was considered but rejected as disproportionate for what the ticket needs (field/text reuse, not media reuse).
  - **Fresh `questionId`s per question** — `questionId` is a result-tracking key, so each cloned question is re-keyed via a `_withFreshQuestionId` helper that round-trips through `toJson()`/`fromJson()` (both already polymorphic per `CardQuestion` subtype) rather than a switch over five near-identical `copyWith` calls.
  - **Set membership is entry-point-dependent**: cloning from the Cards tab (`my_cards_screen.dart`) leaves the clone unattached, matching how any other newly-created card behaves there. Cloning from within a set (`set_detail_screen.dart`'s row quick-actions/context menu) passes the live set as `parentSet` — the exact same mechanism #233 already uses to link a brand-new card to its set on save — so the clone lands in that set automatically. No new linking code was needed for this.
  - **Entry points**: the row quick-actions (#260, third icon alongside Edit/Remove) and right-click context menu (#237) on both the Cards tab and set-detail rows — surfaces #260/#237 explicitly reserved for exactly this. No new UI surface (e.g. the full-screen editor's own toolbar) was added; see the "Open questions" resolution for why.
...
- **Contextual `AppBar`** — replaces the normal one while selecting: **✕**, the selected count, then **Select all** / **Add to set** / **Tag** / **Language** / **Delete**.
- **`SelectionModel`** (`utils/selection.dart`) is the shared, plain (non-`ChangeNotifier`) selection state behind *both* the card library and the set-detail list (#238 part 2 ported it there — see below): `mode`/`selected`/`anchor`, plus `enterWith` / `toggle` / `extendTo` / `toggleSelectAll` / `prune`. It's a mutable object the owning `State` mutates inside `setState`, so rebuild control stays with the widget while the selection logic itself stays trivially unit-tested (`idsInRange` / `toggleId` / `pruneSelection` underneath, plus the model's own tests).
...
- **Bulk delete** — one confirmation covering the count, then the existing per-card `deleteCard` per selected card, so `setCards` links, `cardCount` decrements, and Storage media cleanup all reuse the single-card semantics above rather than a parallel batch path. Because that means N sequential deletes, the rows are hidden **optimistically** (`_pendingDelete`, filtered out of both lists) and selection mode exits immediately, with the deletes running in the background — otherwise the list visibly dissolves a row at a time as each delete returns through the Firestore stream. Only ids that fail are restored to the list, behind an error snackbar; the deletes are allowed to finish even if the user navigates away.
- **Bulk tag** (`widgets/bulk_card_actions.dart`, `_BulkTagDialog`) — a tri-state dialog (ticked/dashed/unticked, `CheckboxListTile(tristate: true)`) built from `utils/bulk_tags.dart`'s pure `tagPresence` (all/some/none across the selection). Ticked tags are added to every selected card, unticked tags are removed from every selected card, dashed (mixed) tags are left alone; a `TagInputField` at the bottom adds brand-new tags (pre-ticked). Applying calls `applyTagDelta` per card, then mirrors the single-card editor's lifecycle exactly — `AppHelpers.diffTags` old-vs-new, then fire-and-forget `tagRepo.upsertTag`/`decrementTag` — so the global tag registry's counts stay accurate across a bulk edit. A card whose delta is empty is skipped entirely (no-op write).
- **Bulk language** (`_BulkLanguageDialog`) — two `LanguagePicker`s (target/native); either can be left untouched to leave that language alone on every card. Since `copyWith` can only *set* a field (`?? this.x`, never null it), this can assign a pair across the selection but can't clear one.
- **Selection maths** is pure and unit-tested in `utils/selection.dart` (`idsInRange` / `toggleId` / `pruneSelection` / `SelectionModel`). Changing the search or tag filter **prunes** the selection to what's still visible, so the count never claims cards the user can't see.

#### Hover states & right-click context menus (#237)

Desktop-native affordances, additive on top of tap-first browsing — touch/mobile is unaffected because `MouseRegion`'s enter/exit and `GestureDetector.onSecondaryTapDown` simply never fire without a mouse; no platform check is needed. This is the first right-click implementation in the app (no `showMenu`/`onSecondaryTapDown` precedent existed before it), so it introduces two small shared primitives used across all three affected screens:

- **`widgets/context_menu.dart`** — `ContextMenuAction` (icon/label/enabled/`destructive`) and `showContextMenu(context, position, actions)`, which opens a `showMenu` positioned at the right-click's coordinates. Non-destructive actions render first; any `destructive`-flagged ones (Delete) are grouped below a `PopupMenuDivider` and coloured with the theme's error colour — separate from `HelpMenuAction` (the ⋮ Help menu's item type, #234) because context menus need this destructive-styling concept and the ⋮ menu never does.
- **`widgets/hover_highlight.dart`** — `HoverHighlight`, a small `StatefulWidget` wrapping a child in `MouseRegion` + `AnimatedContainer` for a subtle background tint on pointer-enter. Used to add hover feedback to the **My Cards** and **My Sets** tiles, which had none before this ticket.

Per screen:

- **My Cards tile** — right-click offers **Edit / Add to set / Tag / Set language / Delete**. Rather than a parallel single-card code path, the existing bulk methods (`_bulkAddToSet`, `_bulkTag`, `_bulkLanguage`, `_bulkDelete`) were generalised to accept an optional `Map<String, String>?` selection override (defaulting to the current multi-select), so a right-click just calls the same method with a one-entry map — a single-card action and a one-item bulk action are, by construction, the same code path.
- **My Sets tile** — right-click mirrors the set-detail toolbar's management actions exactly: **Study / Market toggle / Export / Edit / Delete**. This required extracting `set_detail_screen.dart`'s `_confirmDelete`/`_exportSet`/`_offerInMarket`/`_removeFromMarket`/`_study` (plus the `MarketPublishSheet` bottom sheet, made public) into **`widgets/set_actions.dart`** as standalone `(context, ref, CardSet)` functions — `deleteSetWithConfirm`, `exportSetWithProgress`, `toggleMarketPublish`, `openEditSet`, `openStudySetup`. `set_detail_screen.dart`'s own toolbar methods became thin wrappers around these (preserving their `_isDeleting`/`_isExporting`/`_isPublishing` busy-flag UI), so both call sites do the exact same delete/export/publish flow — no parallel copy to drift.
- **Set Detail rows** — right-click surfaces just **Edit / Remove from set**, the same two actions the hover-reveal quick-actions (#260) already expose; it's a convenience path for users who'd rather right-click than hover-and-click the small icon, not a new capability. The hover **tint** reuses the row's existing `_hoveredCardId` tracking (the same state that reveals the quick-action icons) rather than introducing a second hover mechanism — `selected` (multi-select) tint wins over `hovering` tint, neither means the default `Card` colour. Both the context menu and hover tint are suppressed while `SelectionModel.mode` is active, same as the existing quick-actions/drag/swipe suppression from #238 part 2.
...
**Build status:** data model + serialisation, the pill-mode study renderer (tap-to-fill table), and authoring in the workbook card form (live row/column steppers, optional header toggles, editable cell grid, empty-count) are complete. Authoring is workbook-only for now, matching the other rich types. Reuses the shared `CompletionMode` + tap-to-fill machinery from #170. Text-input mode + the mode selector land with the #168 pass.
...
**Question focus chain (#87 follow-up).** When a card's additional questions are revealed, the first one's input auto-focuses, and answering it auto-focuses the next one — a keyboard-driven review loop shouldn't require reaching for the mouse between questions. Applies to `_WorkbookTextInputCard`, the `textInput` completion mode of `_FillInTheBlanksCard`/`_GridCard` (each field-per-token/cell gets its own `FocusNode`, keyed the same way as their existing `TextEditingController`s), and `_WorkbookMultipleChoiceCard` (the first `_OptionButton`'s underlying `OutlinedButton` gets a `FocusNode`, since Flutter's default button shortcuts already bind Enter *and* Space to activation once a button has focus). `word_order` and pill-mode fill-in-the-blanks/grid have no focusable input to auto-focus, so they're unaffected.

Auto-focusing the button is necessary but not sufficient for Enter: the screen-level `Focus(onKeyEvent: _handleKeyEvent)` sits *between* the focused button and the root-level `WidgetsApp` Activate shortcut, so it sees Enter first and — left alone — swallows it to advance the card. (Space is fine: `_handleKeyEvent` never handles Space, so it bubbles straight through.) The fix is a second guard alongside `_isTextFieldFocused`: `_isButtonFocused()` (the same `findAncestorWidgetOfExactType<OutlinedButton>()` ancestor walk) makes the Enter case return `KeyEventResult.ignored` while an option button holds focus, letting Enter bubble up to the Activate shortcut and select the option. Once the question is answered every option's `onPressed` is null, so the disabled buttons drop focus and Enter resumes advancing. Covered by a second case in `test/screens/study/study_keyboard_shortcuts_test.dart`.
...
- Before starting, user can configure:
  ...
  - **Re-queue missed cards** (#214): persisted as `StudySession.requeueMissed` (default false). When on, the session engine appends a card's ID to the back of `cardSequence` when the user advances off it having **missed** it that visit — any question answered incorrectly, **or** the flashcard's primary word self-evaluated **"Not yet"**. The card keeps reappearing until a visit where it isn't missed (all questions correct / "Knew it"). Appended at most once per visit (`requeueMissedCard` in `utils/study_filters.dart`). The dynamic append rides the normal auto-save path, so a resumed session continues re-queuing.
    - **Metrics are per-visit ("total experiences"), not per unique card.** Every time a card is seen is a distinct graded experience: `totalCardsStudied` is a high-water count of positions visited (re-queued repeats each add one), and the recall tallies (`cardsKnown` / `cardsUnknown`) and question score count once **per sequence position**. Each position is finalized exactly once — on leaving the card (`_finalizeCurrentVisit` for recall; a position-keyed `_countedQuestions` set for questions) — so navigating back then forward to an already-graded position never re-counts, while a re-queued repeat (a new position) does. The summary derives flashcard visits vs workbook visits from `cardSequence.take(totalCardsStudied)`, keeping "known %" consistent with the per-visit denominator.
...
On wide layouts the **My Sets** tab is a **master-detail** view (#236): the set list stays in a left column and the selected set's `SetDetailScreen` renders in a right pane (empty placeholder until a set is chosen). Selecting a set updates the pane in place; narrow/portrait keeps push navigation. `SetDetailScreen` takes an optional `onExit` callback so deleting the set in the pane clears the selection instead of popping a route. The Market tab stays full-width.

Master-detail engages on the **same window-width breakpoint as the nav rail** (`isWideWidth(MediaQuery.sizeOf(context).width)`, not the pane's local width) so the two flip together — there is no in-between state where the rail is present but the set list has collapsed to single-column. Because a just-wide window leaves little room, the set list width is **responsive**: it takes `kSetListWidth` (340) when there's room but shrinks toward `kMinSetListWidth` (260) first, keeping the detail pane at least `kMinDetailPaneWidth` (300) so its card rows never overflow their tiles.

The pane's set-mode toolbar is **adaptive** (#234): when the pane is narrower than `kSetToolbarOverflowWidth` (520) the set-management actions (market/export/edit/delete) fold into the **top of the existing ⋮ Help menu** (`HelpMenuButton` now takes `extraActions`), above a divider before Help/Send Report — one overflow menu, not a second ⋮ — keeping the builder actions (+, library, study) always visible so the AppBar action row can't overflow.

**In-pane card editing (#259)** — the pane's toolbar and body *morph* rather than pushing a new screen: creating a card (toolbar **Add card** button — a `PopupMenuButton` offering Flash/Workbook as a local menu rather than the mobile bottom sheet, #234; the FAB is hidden in the pane, a mobile idiom) or double-clicking an existing row swaps the card list for the embedded `CardEditorBody`/`WorkbookEditorBody` (#255), and the set-mode toolbar (market/export/delete-set/edit-set/study) swaps for **Cancel** / **Save** only — there is deliberately **no delete-card action in the pane**; card deletion remains a Cards-tab-only capability. Save/Cancel are driven from the toolbar via a `GlobalKey<CardEditorBodyState>` / `GlobalKey<WorkbookEditorBodyState>` (`.save()`), with the embedded editors' own footer Cancel/Save row hidden (`showFooterActions: false`) to avoid duplicate controls. A `PopScope` intercepts Android system-back to cancel the pane edit rather than exiting the app. (Rejected: a nested `Navigator` per card edit — it pushes a separate screen with its own AppBar/back button, which can't morph a single shared toolbar the way this UX calls for.)

**Row quick-actions (#260)** — each set-detail row also exposes **Edit** and **Remove-from-set** `IconButton`s next to the reorder handle (`_rowQuickActions`), routed through the same `_editCardRow` helper as double-tap (wide → pane, narrow → full-screen push) and the same `_removeCard` as swipe-to-remove (with Undo, #247) — no new behaviour, just a discoverable, mouse-friendly entry point. On wide/pane layouts the actions **hover-reveal** (`MouseRegion` tracking `_hoveredCardId`, `AnimatedOpacity` + `IgnorePointer` so a hidden button can't steal a tap); on narrow/touch layouts, which have no hover concept, they're always visible — this also gives touch users their first way to edit a card directly from within a set. **Clone** was deliberately omitted at the time (until #231/#274 landed); the actions row was built to take a third button without layout changes, which is exactly what happened. No card-deletion action here — the underlying card is never touched, only its set membership.

**Multi-select in the set list (#238 part 2)** — the set-detail list reuses the same `SelectionModel` as the card library (see the Card CRUD section above), with the two gesture systems already on the row kept from colliding: **entering selection mode disables drag-to-reorder and swipe-to-remove** for every row (the `ReorderableDragStartListener` handle and the `_rowQuickActions`/`Dismissible` wrapper are simply omitted while `_selection.mode` is true; a bare `KeyedSubtree` takes the `Dismissible`'s place so `ReorderableListView` still gets the key it needs), and the row's leading icon swaps to a `Checkbox`. Both return the instant selection mode ends. On the **wide pane**, **Select** is a dedicated toolbar `IconButton` — there's always room, since a narrowing pane folds the market/export/edit/delete actions into the ⋮ menu and frees the space. Only the **narrow/full-screen** toolbar, which has no equivalent fold to lean on, keeps **Select** as a `HelpMenuAction` in the ⋮ menu instead (gated on the list being non-empty); long-press and Ctrl/⌘+click still work directly there without it, matching the card library. The contextual `AppBar` offers **Select all** / **Tag** / **Language** / **Remove from set** — deliberately no **Delete** (card deletion stays Cards-tab-only, per the decision on #260 above) and no **Add to set** (that's the library drawer's job, #234). Bulk **Tag**/**Language** call the exact same `widgets/bulk_card_actions.dart` functions as the card library. Bulk **Remove from set** is one confirmation then a loop of the existing `removeCardFromSet` per selected id — the cards themselves are never touched, only their set membership — absorbing the batch case #247 deferred.

**Library drawer (#234)** — the "Add existing cards" library is a single adaptive widget (`_CardLibrary`) with an `asDrawer` flag. On narrow/full-screen it opens as a `DraggableScrollableSheet` bottom sheet (tap-to-select, **Add**). On wide/pane layouts (`onExit != null`) it instead docks as a fixed-width side panel beside the card list (`_libraryOpen` state) and stays open for repeated batches, with a **✕** to dismiss. In panel mode each library row is a `Draggable<Map<String,String>>` (payload = the hovered card, or the whole current selection if that card is selected) rendered against a `DragTarget` wrapping the card list; dropping calls `_dropCards` → the shared `_addCardsWithLanguageCheck`, which the sheet's **Add** button also uses. Extracting that top-level helper means the #210 language-consistency checks (set-language / mixed / different-language dialogs) and the type-batched `addCardsToSet` live in exactly one place regardless of entry point. Cards are always **appended** (order-preserving); the drop target tints while a drag hovers. The panel is toggled directly from a dedicated toolbar `IconButton` (`_libraryOpen`, `isSelected`/`selectedIcon` to show active state) — the +-menu's "Add existing cards" entry sets the same flag, so on wide the sheet step is skippable. A `LayoutBuilder` guards the docked layout by pane width: at ≥`kLibraryDrawerMinPaneWidth` (620) it docks side-by-side (drag-and-drop available); below that (the roughly square window case) the library **covers the whole pane** instead — multi-select + **Add** still work, drag-and-drop doesn't, and the card rows never get squeezed into an overflow. This completes the #232 in-place set-builder epic.
