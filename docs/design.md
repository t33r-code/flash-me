...
**Clone Card (#231)** — Flash Cards only (Workbook Card cloning is a separate, unstarted follow-up, mirroring the split #269 makes for Save as Template). Clone opens the editor exactly as if creating a brand-new card, seeded from the source's `primaryWord`/`translation`/`questions`/`tags`/language pair — it is a genuinely **unsaved draft**: nothing is written until the author hits Save, and cancelling creates nothing. Implementation-wise this is a single new `cloneFrom: FlashCard?` parameter threaded through `CardEditorBody`/`CardFormScreen` (and `_FlashPaneEdit` for the wide pane), seeded in `initState()` only when `card == null` — so the existing `_isEditing`/`_pendingCardId`/`_save()` branches are completely untouched and the clone naturally goes through `createCard` with a fresh id/`createdAt`/`createdBy`, never touching the source.
