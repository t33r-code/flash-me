...
- [x] Authoring UI (workbook card form): sentence → Tokenize → tap-to-mark-eligible → blank count stepper + distractor words; pill mode only (text-input mode + mode selector arrive with #168). Flash-card form and question templates deferred (word_order is likewise workbook-only today)
...
- [x] Remove/detach with Undo (#247): swipe-to-remove now offers a snackbar **Undo** that re-links the card at its original `position` (`addCardToSet(..., position:)`). Single-card scope; batch/multi-select remove moved to #238 (shared selection model). Position order stays correct after removals via the sort's addedAt tie-break — no re-compaction needed.
...
- [x] #233 — create a card directly in a set: the set's **+** sheet offers "Flash Card" / "Workbook Card" (opens the full-screen editor seeded with the set) or "Add existing cards" (picker). The editors link the new card to `parentSet` on create (`addCardToSet`), so create + add is one step. Desktop pane editing is still #236.
- [x] #236 — multi-pane = desktop detail-pane editing (hosts `CardEditorBody`)
- [x] #234 — library drawer (replaces the #210 picker): on wide/pane layouts the "Add existing cards" library opens as a docked side panel (`_CardLibrary(asDrawer: true)`) instead of a bottom sheet, and stays open for repeated batches. Cards (or a whole selection) can be **dragged** from the panel and **dropped** onto the set list (`Draggable` → `DragTarget` → `_dropCards`), which highlights while a drag hovers. Narrow layouts keep the `DraggableScrollableSheet` bottom sheet (`asDrawer: false`, tap-to-add). Both paths share the extracted `addCardsWithLanguageCheck` (#210 language consistency checks + batched `addCardsToSet`), since #238 promoted to `widgets/add_cards_to_set.dart`. Completes the #232 in-place set-builder epic.
...
- [x] #236 — multi-pane set view: master-detail (My Sets tab splits into set list + detail pane on wide; reuses `kWideLayoutBreakpoint`). `SetDetailScreen` gained an optional `onExit` callback so pane deletes clear the selection instead of popping. Card editing lands in-pane via #259.
- [x] #259 — in-pane card editing (Option B, morphing pane): `SetDetailScreen`'s pane toolbar/body swap in place — set actions ↔ Cancel/Save, card list ↔ embedded `CardEditorBody`/`WorkbookEditorBody` (`showFooterActions: false`, saved via a `GlobalKey`). Add card is a toolbar button (FAB hidden on wide); double-clicking a row opens it for editing. No card-delete in the pane (Cards tab only). `PopScope` cancels the pane edit on Android system-back. Per-row Edit/Remove/Clone quick-action buttons are the follow-up (#260). Follows #236.
- [x] #260 — per-row quick-actions: each row gets Edit + Remove-from-set `IconButton`s next to the drag handle, hover-reveal on wide/pane (`MouseRegion` + `AnimatedOpacity`/`IgnorePointer`), always-visible on narrow/touch. Edit routes through the same wide→pane / narrow→full-screen split as double-tap; Remove reuses #247's `_removeCard` + Undo. Clone deferred until #231 lands. No card-deletion action added.
- [x] #235 — app-wide keyboard support & polish for desktop (parent; its own scope — study shortcuts, Enter/Esc consistency, editor focus/tab-order — is complete. Global-shortcut ideas live on as independent backlog tickets: #278 Ctrl/Cmd+S, #279 `/` focus-search, #280 Ctrl/Cmd+N create, #281 `?` cheatsheet)
  - [x] Enter-to-confirm / Esc-to-cancel: new shared `widgets/keyboard_actions.dart` (`KeyboardActions` — a `Focus(onKeyEvent:)` wrapper taking optional `onConfirm`/`onCancel`). A probe test first confirmed Flutter already dismisses `barrierDismissible` dialogs on Esc in this version, so the real gaps were the five full-screen editors (routes → no Esc) and Enter (nothing confirmed anywhere). Esc-to-cancel added to the flash-card/workbook-card/set/template/question-template editors (Enter-to-submit deliberately left to Ctrl/Cmd+S #278 since they're multi-field/multi-line). Enter-to-confirm added to the confirmation/choice dialogs (delete card/set/account/template, remove-from-set, remove-from-market, unlink, replace-questions, import preview/summary, add-to-set language dialogs, bulk-language) plus `onFieldSubmitted` on the forgot-password and link-email entry dialogs. Enter defers (bubbles) while a focused button or a multi-line field has focus; `onCancel` passed only where Esc isn't already handled, to avoid a double-pop. Bulk-*tag* dialog left alone (its tag field owns Enter; Esc already dismisses). Covered by `test/widgets/keyboard_actions_test.dart`.
  - [x] Focus visibility / tab order in the editors: audit found both editors are already keyboard-focusable in reading order (standard Material widgets + `InkWell`-based `LanguagePicker`); the one real gap was the set editor's colour picker, built from bare `GestureDetector`s with no keyboard path. Extracted the swatches into `widgets/color_picker_swatch.dart` (`ColorPickerSwatch` — `FocusableActionDetector` for tab-traversal + Enter/Space activation, `GestureDetector` for pointer taps, a 2 dp primary-colour focus ring shown only on keyboard focus via `onShowFocusHighlight`). The card editor's image-thumbnail `GestureDetector` was left as-is (it has a redundant focusable Add/Replace-image button). Covered by `test/widgets/color_picker_swatch_test.dart`.
  - [x] #87 — desktop study shortcuts: `StudySessionScreen` wraps its `Scaffold` in `Focus(autofocus: true, onKeyEvent:)`. ←/→ mirror the nav-bar arrows (previous/next, regardless of reveal state); Enter reveals everything at once (translation + any extra questions, one press) then advances on the next press — collapsing what's normally two taps into Enter's two-stage "reveal / advance" rather than mirroring every tap step-for-step; K/2 and U/1 mark Skip/Review (digits follow the nav bar's Review-then-Skip display order rather than pairing with their letter). Every shortcut is gated behind `_isTextFieldFocused()` so typing a literal `k`/`u` (or moving the cursor with arrow keys) while answering a text-input question is never hijacked as a shortcut — the naive `primaryFocus?.context?.widget is EditableText` check silently never matches (that context's widget is the `Focus` EditableText wraps itself in, not EditableText), a bug caught by a new isolated widget test (`test/screens/study/study_keyboard_shortcuts_test.dart`) before it shipped; the fix walks the ancestor chain instead. Nav-bar tooltips get inline shortcut hints (`(←)`, `(K / 2)`, etc.) at the call site rather than editing the shared l10n strings. Follow-up: `_WorkbookTextInputCard` and the `textInput` mode of `_FillInTheBlanksCard`/`_GridCard` auto-focus their first field on reveal and the next one after each answer, via each widget's own `initState` calling `FocusNode.requestFocus()` in a post-frame callback — `TextField.autofocus` doesn't work here since it only claims focus if nothing already has it, and the screen's own `Focus(autofocus: true)` always claims it first. Second follow-up: `_WorkbookMultipleChoiceCard` gets the same auto-focus on its first `_OptionButton`'s `OutlinedButton`, so Enter (which Flutter's default button shortcuts already bind alongside Space) selects the focused option instead of being swallowed by the screen-level reveal/advance shortcut.
- [x] #237 — hover states & right-click context menus: the app's first right-click implementation, via two new shared primitives — `widgets/context_menu.dart` (`ContextMenuAction` + `showContextMenu`, `showMenu` positioned at the click, destructive actions grouped below a divider) and `widgets/hover_highlight.dart` (`HoverHighlight`, a `MouseRegion` + `AnimatedContainer` tint). Both are pointer-only by construction (`MouseRegion`/`onSecondaryTapDown` never fire on touch), so mobile is unaffected without a platform check. My Cards tiles get Edit/Add-to-set/Tag/Set-language/Delete (the existing #238 bulk methods generalised to accept an explicit selection map, so a single right-click reuses the exact same code as a one-item bulk action); My Sets tiles get Study/Market-toggle/Export/Edit/Delete, which required extracting `set_detail_screen.dart`'s equivalent toolbar logic into standalone `widgets/set_actions.dart` functions (`deleteSetWithConfirm`, `exportSetWithProgress`, `toggleMarketPublish`, `openEditSet`, `openStudySetup`) that both screens now share; set-detail rows get Edit/Remove-from-set, mirroring the #260 hover-reveal quick-actions rather than adding new capability, with the hover tint reusing that same `_hoveredCardId` state. Both suppressed while #238's `SelectionModel.mode` is active.

##### Phase 4h — Bulk authoring (Alpha 0.7)
- [x] #238 part 1 — multi-select + bulk add-to-set / delete in the card library: explicit selection mode (Select toolbar button, long-press on touch, Ctrl/⌘+click on desktop; only ✕ exits). In mode, plain tap toggles and **Shift+click unions the anchor→target span** into the selection (non-destructive — extending never silently discards earlier picks). A contextual `AppBar` replaces the normal one, showing the count plus Select all / Add to set / Delete. Bulk add-to-set picks a target set (`_SetPickerDialog`) then reuses the shared `addCardsWithLanguageCheck`; bulk delete runs the existing per-card `deleteCard` behind one count confirmation, so set links + Storage media cleanup come for free. Selection maths lives in pure `utils/selection.dart` (`idsInRange` / `toggleId` / `pruneSelection`, 13 tests); the filter chips + search prune the selection so the count stays honest. Promotes `addCardsWithLanguageCheck` out of `set_detail_screen` into `widgets/add_cards_to_set.dart` so the set builder and the card library share one #210 path.
- [x] #238 part 2 — bulk **Tag** add/remove, **Set language pair**, and bulk **Remove from set** (absorbed from #247). Promotes the selection state itself into a shared `SelectionModel` (`utils/selection.dart`) used by both the card library and the set-detail list. The set list resolves the reorder/swipe conflict by disabling both while selecting: the drag handle and `_rowQuickActions`/`Dismissible` are simply omitted, leaving a bare `KeyedSubtree` so `ReorderableListView` still gets its key; both return the moment selection mode ends. No dedicated **Select** button on the set toolbar (already tight, up to 6 icons on narrow) — it's a `HelpMenuAction` in the existing ⋮ menu instead. Tag/Language dialogs (`widgets/bulk_card_actions.dart`) and their pure helpers (`utils/bulk_tags.dart` — `tagPresence`, `applyTagDelta`) are shared by both screens; bulk tag mirrors the single-card editor's `upsertTag`/`decrementTag` lifecycle exactly via `AppHelpers.diffTags`, so the global tag registry stays accurate. Set-list bulk actions omit Delete (Cards-tab-only, #260) and Add-to-set (the library drawer's job, #234). Closes #238.
- [ ] #241 / #242 / #243 — paste-to-create
- [ ] #239 — templates in the set builder
- [ ] #240 — duplicate detection
- [x] #231 — clone card (Flash Cards). New `cloneFrom: FlashCard?` param on `CardEditorBody`/`CardFormScreen` (and `_FlashPaneEdit` for the wide pane) seeds a brand-new draft — `card` stays null, so the untouched create path (`createCard`, fresh id/`createdAt`/`createdBy`) runs and cancelling writes nothing. Media is deliberately not copied (no reference counting on `deleteCard`, so a shared URL would let deleting either card break the other's); questions get fresh `questionId`s via a `toJson`/`fromJson` round-trip. Cloning from within a set passes the live set as `parentSet` — reusing #233's existing new-card-linking mechanism — so it lands in that set automatically; cloning from the Cards tab leaves it unattached. Entry points: the row quick-actions (#260, third icon) and right-click context menu (#237) on both the Cards tab and set-detail rows — the surfaces both tickets explicitly reserved for this.
- [x] #274 — clone card (Workbook Cards), mirroring #231. Same `cloneFrom` mechanism on `WorkbookEditorBody`/`WorkbookCardFormScreen` and `_WorkbookPaneEdit`. The fresh-questionId helper (`_withFreshQuestionId`) was promoted out of `card_form_screen.dart` into a shared top-level `withFreshQuestionId` in `models/card_question.dart` so both editors' clone paths re-key identically instead of keeping separate copies. `_cloneCardRow`/`_cardContextActions`/`_rowQuickActions` in `set_detail_screen.dart` and `my_cards_screen.dart` — previously Flash-Card-gated — now branch on card type instead, so Clone is a single unconditional slot in every menu/row rather than two parallel implementations. Workbook Cards have no media fields, so the media-orphaning question #231 had to resolve doesn't apply here. Closes #274.
...
- [x] Create architecture documentation — code map and key decisions in `CLAUDE.md`; full design rationale in `docs/design.md`
...
- [x] Set up app store deployment process (Google Play Console — app created, internal test track live, version 0.2.0+1 uploaded)
...
- [ ] Implement in-app account deletion — cascade delete all Firestore data (cards, sets, setCards, templates, study sessions), Firebase Storage files, user document, and Firebase Auth account; required before moving beyond internal testing (currently handled via email request to t33r.code@gmail.com)
