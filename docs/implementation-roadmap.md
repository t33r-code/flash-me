...
- [ ] #235 — app-wide keyboard support & polish for desktop
  - [x] #87 — desktop study shortcuts: `StudySessionScreen` wraps its `Scaffold` in `Focus(autofocus: true, onKeyEvent:)`. ←/→ mirror the nav-bar arrows (previous/next, regardless of reveal state); Enter reveals everything at once (translation + any extra questions, one press) then advances on the next press — collapsing what's normally two taps into Enter's two-stage "reveal / advance" rather than mirroring every tap step-for-step; K/1 and U/2 mark Skip/Review. Every shortcut is gated behind `_isTextFieldFocused()` so typing a literal `k`/`u` (or moving the cursor with arrow keys) while answering a text-input question is never hijacked as a shortcut — the naive `primaryFocus?.context?.widget is EditableText` check silently never matches (that context's widget is the `Focus` EditableText wraps itself in, not EditableText), a bug caught by a new isolated widget test (`test/screens/study/study_keyboard_shortcuts_test.dart`) before it shipped; the fix walks the ancestor chain instead. Nav-bar tooltips get inline shortcut hints (`(←)`, `(K / 1)`, etc.) at the call site rather than editing the shared l10n strings.
