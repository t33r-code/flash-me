...
- [x] #234 — library drawer (replaces the #210 picker): on wide/pane layouts the "Add existing cards" library opens as a docked side panel (`_CardLibrary(asDrawer: true)`) instead of a bottom sheet, and stays open for repeated batches. Cards (or a whole selection) can be **dragged** from the panel and **dropped** onto the set list (`Draggable` → `DragTarget` → `_dropCards`), which highlights while a drag hovers. Narrow layouts keep the `DraggableScrollableSheet` bottom sheet (`asDrawer: false`, tap-to-add). Both paths share the extracted top-level `_addCardsWithLanguageCheck` (#210 language consistency checks + batched `addCardsToSet`). Completes the #232 in-place set-builder epic.
