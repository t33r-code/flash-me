...
// Firestore implementation of StudySessionRepository.
class FirebaseStudySessionRepository implements StudySessionRepository {
  final FirebaseFirestore _firestore = FirebaseFirestore.instance;
  final Logger _logger = Logger();

  // Convenience: returns the subcollection ref for a user's sessions.
  ...
  Future<StudySession> createSession(
      StudySession session, String userId) async {
    try {
      final docRef = _sessionsRef(userId).doc();
      final now = DateTime.now();
      final newSession =
          session.copyWith(id: docRef.id, startTime: now, lastAccessTime: now);
      await docRef.set(newSession.toFirestore());
      _logger.i('Created session ${docRef.id}');
      return newSession;
    } catch (e) {
      _logger.e('Failed to create session: $e');
      throw AppException('Failed to start session',
          code: 'create-session-failed');
    }
  }
  ...
  Future<void> saveSession(StudySession session, String userId) async {
    try {
      ...
      await _sessionsRef(userId).doc(session.id).update(updated.toFirestore());
    } catch (e) {
      _logger.e('Failed to save session ${session.id}: $e');
      throw AppException('Failed to save progress', code: 'save-session-failed');
    }
  }
  ...
  Stream<List<StudySession>> watchSessionHistory(String setId, String userId) {
    return _sessionsRef(userId)
        ...
        .snapshots()
        .map((s) => s.docs.map(StudySession.fromFirestore).toList());
  }
}
